...
def _redaction_patterns():
    ...
    patterns = [
        ...
        # Spend and plan state, kept in step with the other two lists this
        # module's siblings maintain. This file writes prose to disk, so a
        # pasted spend total or account field would land in the candidates
        # file verbatim -- every identity pattern above is structurally blind
        # to a bare number.
        (re.compile(r"\$\d{1,3}(,\d{3})+(\.\d{2})?|\$\d{4,}(\.\d{2})?"), "<amount>"),
        # Literals split across adjacent string pieces so this file does not
        # match the pattern it defines, and held to the subset of regex syntax
        # both grep -E and Python re accept, so all three lists stay identical.
