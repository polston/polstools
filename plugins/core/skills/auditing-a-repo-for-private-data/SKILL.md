...
and a scrub is verified only when **all six read zero**.
...
| 5 | Author/committer metadata | `git log --all --format='%an <%ae> %cn <%ce>'` |
| 6 | Files ever added | `git log --all --diff-filter=A --name-only` |

A `^\+`-filtered patch sweep is blind to places 1, 4, and 5. That exact gap let
an account name survive two pushes after a scrub reported zero hits.
...
If it is clean, say which six checks were run and that each read zero — a bare
"clean" is not a result anyone can act on.
