...
description: Use when the format plugin misbehaves — replies stop following FINDINGS/PROBLEMS/ASKS, injected text is missing or mangled, /format:off or /format:on seems stuck — after editing its wiring or payloads, or when disabling or removing the plugin for good.
---
...
1. Run `python <plugin-root>/bin/format-e2e`. Exit 0 = the wiring, gates,
   toggle, and catalogs all check out mechanically. Exit 1 names the failing
   check.
2. The e2e cannot see live state. Check separately: is the plugin enabled in
...
1. Uninstall/disable `format` in the harness (`/plugin`), or remove its entry
   from both catalogs if deleting from the repo.
...
3. Remove any instruction-file carve-out that defers other formatting rules
   to this plugin (grep the user/global instructions for `format`).
